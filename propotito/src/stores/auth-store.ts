...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      login: async (credentials: LoginRequestDTO) => {
        ...
        try {
          ...
          // Obtener datos del usuario actual con avatar incluido
          let currentUser = null;
          try {
            currentUser = await usersApi.me();
            console.log('[AuthStore] User fetched with avatar:', currentUser.avatar);
          } catch (e) {
    ...
    }),
    {
      ...
      version: STORE_VERSION, // Migración automática si cambia la versión
      ...
      migrate: (persistedState: any, version: number) => {
        if (version !== STORE_VERSION) {
          console.log('[AuthStore] Migrating from version', version, 'to', STORE_VERSION);
